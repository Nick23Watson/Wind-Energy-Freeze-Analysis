...
% ERA5 file
TxERA5 = "Working\Data\Raw\TX-ERA5-FEB-1-28-2021.grib";

% Station location (match ASOS station)
stationName = "Victoria Regional Airport";
stationLat  = 28.86245;
stationLon  = -96.9301;
...
grid on;
ylabel('Temp (°C)');
title(sprintf('ERA5 Single-Point Icing Analysis\n%s', stationName), ...
      'FontWeight','bold');
...
sgtitle(sprintf(['ERA5 Icing Identification\n', ...
    '%.2f°N %.2f°E | %s to %s | Total Icing Hours = %d'], ...
...
outDir = fullfile(pwd,'Working','Results','ERA5-Regional-TX');
if ~exist(outDir,'dir'), mkdir(outDir); end

saveas(gcf, fullfile(outDir, ...
    sprintf('ERA5_%s.png', ...
    replace(stationName,' ','_'))));
