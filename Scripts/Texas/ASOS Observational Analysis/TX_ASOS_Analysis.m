...
%% Load CSV and set thresholds
asosFile = "4186137.csv";    % set to your uploaded CSV name
outDir   = fullfile(pwd, "Working", "Results", "TX-ASOS");
if ~exist(outDir,'dir'), mkdir(outDir,'s'); end

% Icing rule: T <= 0°C AND RH >= 90% AND wspd >= 2 m/s
windSpeedMin = 2;   
...
%% Form Icing Data arrays
%   (FINAL): icing if TMP <= 0°C AND RH >= 90% AND WSPD >= threshold
...
%% Place data in bins with respective stations
...
%% Interpolate map - start generation
...
TX = states(strcmp({states.Name},'Texas'));

%% Mask outside Texas
mask = inpolygon(LonG, LatG, TX.Lon, TX.Lat);
Igrid(~mask) = NaN;
...
plotm(TX.Lat, TX.Lon, 'k', 'LineWidth', 1.5);

% Overlay actual ASOS points
scatterm(LAT, LON, 60, Ihours, 'filled', 'MarkerEdgeColor','k');

% Add labels
% Choose key stations to label (example: top 20% icing)
th = prctile(Ihours,90);
label_idx = Ihours >= th;

for i = 1:height(Results)
    if ~isnan(Results.LAT(i)) && ~isnan(Results.LON(i)) && label_idx(i)
        textm(Results.LAT(i)+0.07, Results.LON(i), char(Results.NAME(i)), ...
            'FontSize', 10, 'FontWeight', 'bold', 'Color','k');
    end
end


title({'ASOS Observed Icing Hours (Feb 14–17, 2021), 2m/s', ...
       ''}, ...
       'FontSize', 16);

%% Save
saveas(gcf, fullfile(outDir,'TX_ASOS_2ms_Storm.png'));
fprintf("Saved map: %s\n", fullfile(outDir,'ASOS_ASOS_2ms_Storm.png'));
